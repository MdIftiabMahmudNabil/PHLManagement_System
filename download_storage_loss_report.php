...
if (isset($_GET['format']) && in_array($_GET['format'], ['pdf', 'doc'])) {
    ...
    $sql = "SELECT 
                hb.batchID,
                (hb.storedInitialAmount - hb.storedFinalAmount) AS lossAmount,
                hbi.issueProblem AS lossCause
            FROM tbl_harvest_batch AS hb
            LEFT JOIN tbl_harvest_batch_issue AS hbi 
                ON hb.batchID = hbi.batchID AND hbi.stageIssue = 'Storage'";

    $result = $conn->query($sql);

    if ($result->num_rows > 0) {
        $reportContent = "<h1>Storage Loss Report</h1>";
        ...
        if ($format === 'pdf') {
            ...
            $dompdf->render();
            $dompdf->stream('Storage_Loss_Report.pdf');
        } elseif ($format === 'doc') {
            header("Content-type: application/vnd.ms-word");
            header("Content-Disposition: attachment;Filename=Storage_Loss_Report.doc");
            echo $reportContent;
        }
    } else {
